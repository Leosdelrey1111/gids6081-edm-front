...
export const TaskModal = ({ isOpen, onOpenChange, task, userId, onSave }: Props) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (task) setForm({ name: task.name, description: task.description, priority: task.priority });
  ...
  const set = (field: 'name' | 'description') => (e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  return (
    <GenericModal
      ...
      body={
        <form id="task-form" onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
          {apiError && <Alert message={apiError} />}
          <Input id="task-name" label="Nombre" value={form.name} onChange={set('name')} error={errors.name} maxLength={150} required />
          <Input id="task-desc" label="Descripción" value={form.description} onChange={set('description')} error={errors.description} maxLength={200} required />
          <Checkbox
      ...
      footer={
        <>
          <Button variant="flat" className="bg-zinc-800 text-zinc-300 hover:bg-zinc-700" onPress={() => onOpenChange(false)}>
            Cancelar
          </Button>

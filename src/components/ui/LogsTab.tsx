...
import { Chip, Spinner, useDisclosure, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter, Button, addToast } from '@heroui/react';
...
interface LogsTabProps { pathFilter: string; }
...
export const LogsTab = ({ pathFilter }: LogsTabProps) => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { setLogs(await logsService.getByPath(pathFilter)); }
    catch (err) { addToast({ title: 'Error', description: err instanceof Error ? err.message : 'Error al cargar logs.', color: 'danger' }); }
  ...
  const renderCell = (log: Log, key: string) => {
    switch (key) {
      case 'path':       return <span className="font-mono text-xs text-foreground">{log.path}</span>;
      case 'statusCode': return <StatusBadge code={log.statusCode} />;
      case 'timestamp':  return <span className="text-default-400 text-xs">{new Date(log.timestamp).toLocaleString()}</span>;
      case 'actions':    return <ActionButton typeButton="view" onPress={() => { setSelected(log); onOpen(); }} />;
      default: return null;
    }
  };
  ...
  return (
    <>
      {loading
        ? <div className="flex justify-center items-center py-16"><Spinner color="secondary" size="sm" /></div>
        : <GenericTable data={logs} columns={COLUMNS} renderCell={renderCell} defaultSortColumn="timestamp" />
      }
        ...
        <ModalContent>
          <ModalHeader className="flex items-center gap-3 pb-2">
            ...
            <div>
              ...
              <p className="text-xs text-default-400 font-normal">{selected?.path}</p>
            </div>
          </ModalHeader>
            ...
            {selected && (
              <div className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1 p-3 rounded-lg bg-default-100">
                  ...
                  <span className="text-sm font-mono break-all text-foreground">{selected.path}</span>
                </div>
                {selected.errorCode ? (
